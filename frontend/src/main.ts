...
type View = 'dashboard' | 'alerts' | 'playbooks';
type DetailTab = 'overview' | 'timeline' | 'raw';
type SortKey = 'severity' | 'alert_name' | 'source_ip' | 'service_name' | 'honeypot_type' | 'status' | 'last_seen';
...
function alertsToolbar(): string {
  ...
  return `<input id="search" class="field-input" value="${esc(state.search)}" placeholder="Search IP, service, description..."/><select id="severity" class="field-select">${sevOptions}</select><select id="status" class="field-select">${statusOptions}</select>`;
}
...
function detailPanel(alert: Alert): string {
  ...
  const phases = Array.isArray(meta.phases) ? meta.phases as string[] : undefined;
  ...
  if (state.detailTab === 'overview') {
    ...
    <p style="margin-top:14px;color:var(--text-muted);font-size:11.5px;line-height:1.5">${esc(alert.description || 'No description available.')}</p>`;
  } else if (state.detailTab === 'timeline') {
    body = `<div class="timeline-list">${events.map((event, index) => `<div class="timeline-row"><div class="timeline-dot">${index + 1}</div><div><b>${esc(event.label)}</b><p>${esc(event.detail)}</p><time>${event.time ? new Date(event.time).toLocaleString() : esc(event.source)}</time></div></div>`).join('')}</div>${phases ? '' : ''}`;
  } else {
  ...
  const rows = state.playbooks.length
    ? state.playbooks.map((pb) => {
        ...
        return `<tr><td colspan="5" style="padding:0">
          <details>
            <summary style="cursor:pointer;padding:9px 10px;list-style:none;display:grid;grid-template-columns:1fr 1.4fr 1.6fr 1fr auto;gap:10px;align-items:center">
              <span class="name">${esc(pb.name)}</span>
              <span class="mono" style="color:var(--text-muted);font-size:11px">${esc(trigger)}</span>
              <span class="mono" style="color:var(--text-muted);font-size:11px">${esc(actionsSummary)}</span>
...
function render(): void {
  ...
  app.innerHTML = `<div class="shell">${sidebar()}<main class="main">${view}</main></div>${settingsModal()}`;
  bindEvents();
}

function bindEvents(): void {
  document.querySelectorAll<HTMLElement>('[data-view]').forEach((el) => el.onclick = () => { state.view = el.dataset.view as View; render(); });
  document.querySelectorAll<HTMLElement>('[data-action="settings"]').forEach((el) => el.onclick = () => { state.settingsOpen = true; render(); });
  document.querySelectorAll<HTMLElement>('[data-action="close-settings"]').forEach((el) => el.onclick = () => { state.settingsOpen = false; render(); });
  document.querySelectorAll<HTMLElement>('[data-action="modal-overlay"]').forEach((el) => el.onclick = (event) => { if (event.target === el) { state.settingsOpen = false; render(); } });
  document.querySelectorAll<HTMLElement>('[data-action="save-settings"]').forEach((el) => el.onclick = saveSettings);
  document.querySelectorAll<HTMLElement>('[data-action="sign-out"]').forEach((el) => el.onclick = signOut);
  document.querySelectorAll<HTMLElement>('[data-action="refresh"]').forEach((el) => el.onclick = () => fetchAll());
  document.querySelectorAll<HTMLElement>('[data-action="toggle-live"]').forEach((el) => el.onclick = () => { state.live = !state.live; render(); });
  ...
  document.querySelector<HTMLSelectElement>('#status')?.addEventListener('change', (event) => { state.status = (event.target as HTMLSelectElement).value; render(); });

  document.querySelectorAll<HTMLElement>('[data-sort]').forEach((el) => el.onclick = () => {
  ...
  document.querySelectorAll<HTMLElement>('[data-row-id]').forEach((el) => el.onclick = (event) => {
    if ((event.target as HTMLElement).closest('[data-check-id]')) return;
    selectAlert(el.dataset.rowId || null);
  });
  document.querySelectorAll<HTMLElement>('[data-goto-alert]').forEach((el) => el.onclick = () => { state.view = 'alerts'; selectAlert(el.dataset.gotoAlert || null); });
  document.querySelectorAll<HTMLElement>('[data-action="close-detail"]').forEach((el) => el.onclick = () => selectAlert(null));
  document.querySelectorAll<HTMLElement>('[data-tab]').forEach((el) => el.onclick = () => { state.detailTab = el.dataset.tab as DetailTab; render(); });

  document.querySelectorAll<HTMLInputElement>('[data-check-id]').forEach((el) => el.onclick = (event) => {
    event.stopPropagation();
    const id = el.dataset.checkId!;
    if (state.selectedIds.has(id)) state.selectedIds.delete(id); else state.selectedIds.add(id);
    render();
  });
  document.querySelector<HTMLInputElement>('#select-all')?.addEventListener('change', (event) => {
  ...
  document.querySelectorAll<HTMLElement>('[data-action="bulk-ack"]').forEach((el) => el.onclick = bulkAcknowledge);

  document.querySelectorAll<HTMLElement>('[data-action="ack"]').forEach((el) => el.onclick = acknowledge);
  document.querySelectorAll<HTMLElement>('[data-action="contain"]').forEach((el) => el.onclick = containSource);
  document.querySelectorAll<HTMLElement>('[data-action="resolve"]').forEach((el) => el.onclick = resolveIncident);
}
...
function saveSettings(): void {
  const url = document.querySelector<HTMLInputElement>('#settings-url')?.value.trim() || '';
  const token = document.querySelector<HTMLInputElement>('#settings-token')?.value.trim() || '';
